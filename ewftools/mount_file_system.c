     ...
     libcerror_error_t **error )
{
	...
	if( file_system->ewf_handle != NULL )
	{
...
int mount_file_system_set_handle(
     ...
     libcerror_error_t **error )
{
	static char *function = "mount_file_system_set_handle";
	...
	file_system->ewf_handle = ewf_handle;
...
int mount_file_system_get_handle(
     ...
     libcerror_error_t **error )
{
	static char *function = "mount_file_system_get_handle";
	...
	*ewf_handle = file_system->ewf_handle;
     ...
     libcerror_error_t **error )
{
	 ...
	 || ( handle_index >= 1 ) )
	{
		libcerror_error_set(
		 ...
		 "%s: invalid handle index value out of bounds.",
		 function );
     ...
     libcerror_error_t **error )
{
	...
	if( ( handle_index < 0 )
	 || ( handle_index >= number_of_handles ) )
	{
		return( 0 );
	}
	*ewf_handle = file_system->ewf_handle;
     ...
     libcerror_error_t **error )
{
	...
	static char *function                        = "mount_file_system_get_ewf_file_entry_path_from_path";
	...
	while( path_index < path_length )
	{
		...
		if( unicode_character == (libuna_unicode_character_t) escape_character )
		{
			...
			else
			{
				...
				safe_ewf_file_entry_path[ ewf_file_entry_path_index++ ] = hex_value;
			}
		}
#if !defined( WINAPI )
     ...
     libcerror_error_t **error )
{
	...
	if( mount_file_system_get_ewf_file_entry_path_from_path(
	     file_system,
	     path,
	     path_length,
	     &ewf_file_entry_path,
...
/* Retrieves the filename from a file entry
 * Returns 1 if successful or -1 on error
 */
