...
void (*ewftools_signal_signal_handler)( ewftools_signal_t ) = NULL;
             ...
             unsigned long signal )
{
	...
	switch( signal )
	{
		...
		case CTRL_C_EVENT:
			if( ewftools_signal_signal_handler != NULL )
			{
				ewftools_signal_signal_handler(
				 signal );
			}
			return( TRUE );
...
{
	static char *function = "ewftools_signal_attach";
...
#if defined( _MSC_VER )
	ewftools_signal_initialize_memory_debug();
#endif
     ...
     libcerror_error_t **error )
{
	...
	ewftools_signal_signal_handler = NULL;
